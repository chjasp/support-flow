...
class CloudSqlRepository:
    ...
    def get_document_chunks_range(self, doc_id: str, start_index: int, end_index: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logging.error(f"Invalid UUID format: {doc_id}")
            return []
        ...
        try:
            with self._get_conn() as conn:
                try:
                    cur = conn.cursor()
                    cur.execute(sql, (doc_uuid, start_index, end_index))
                    rows = cur.fetchall()
                    colnames = [desc[0] for desc in cur.description]
                    ...
                    for row in rows:
                        row_dict = dict(zip(colnames, row))
                        results.append({
                            "chunk_id": str(row_dict.get("id")),
                            "doc_id": str(row_dict.get("doc_id")),
                            "doc_filename": row_dict.get("doc_filename"),
                            "chunk_order": row_dict.get("chunk_index"),
                            "chunk_text": row_dict.get("text"),
                            "summary": "",
                        })
                finally:
            ...
            logging.error(f"Error getting document chunks range: {e}", exc_info=True)
